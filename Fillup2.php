<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      if (empty($WMSU_ID) || empty($NAME) || empty($course) || empty($AGE) || empty($ADDRESS) || empty($CONTACT) || empty($Yearlvl) || empty($Birthdate) || empty($CIVILSTAT) || empty($WMSU_EMAIL) || empty($GENDER) || empty($nationality) || empty($XTNAME) || empty($XTCONTACT) || empty($XTADDRESS) || empty($course) || empty($DIAGNOSIS) || empty($ILLNESS) || empty($VACCINE) || empty($DRUG) || empty($PAST) || empty($FAMILY) || empty($XTFAMILY)){
          $errorMessage = "All the fields are required";
      } else {
          $sql = "INSERT INTO clients (WMSU_ID, name, course, age, ADDRESS, CONTACT,  Yearlvl, Birthdate, CIVILSTAT, WMSU_EMAIL, gender, nationality, XTNAME, XTCONTACT, XTADDRESS, XTRELATION, DIAGNOSIS, ILLNESS, VACCINE, DRUG, PAST, FAMILY, XTFAMILY) VALUES ('$WMSU_ID', '$NAME', '$course', '$AGE', '$ADDRESS', '$CONTACT', '$Yearlvl',  '$Birthdate', '$CIVILSTAT',  '$WMSU_EMAIL', '$GENDER', '$nationality', '$XTNAME', '$XTCONTACT', '$XTADDRESS', '$XTRELATION', '$DIAGNOSIS', '$ILLNESS', '$VACCINE', '$DRUG', '$PAST', '$FAMILY', '$XTFAMILY')";
